...
type NotionResultArray = Array<PageObjectResponse | PartialPageObjectResponse | PartialDatabaseObjectResponse | DatabaseObjectResponse>;
type NotionResultObject = PageObjectResponse | PartialPageObjectResponse | PartialDatabaseObjectResponse | DatabaseObjectResponse;
...
export class NotionRepository implements INotionRepository {
    ...
    getLeagueData = async (): Promise<void> => {
        ...
        await this.queryAllDatabaseRows();

        const allDifferentDaysNames = this.getAllDifferentDaysNames();

        // ["Jornada 1", "Jornada 2", "Jornada 3"]
        allDifferentDaysNames.forEach(dayName => {
